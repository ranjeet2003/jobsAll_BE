...
const AnswerKeyModel = require("../models/answerKeys");
require("dotenv").config({ path: "./config.env" });

const postAnswerKey = async (req, res, next) => {
  console.log(req.body);
  try {
    const newAnswerKey = await AnswerKeyModel.create({
...
const getAllAnswerKeys = async (req, res, next) => {
  ...
  res.status(200).json({ status: "success", data: answerkeys });
};

exports.postAnswerKey = postAnswerKey;
exports.getAllAnswerKeys = getAllAnswerKeys;
